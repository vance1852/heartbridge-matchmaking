...
func (d *Dispatcher) deliver(ctx context.Context, job notify.Job) error {
	...
	if sendErr == nil {
		if err := d.repo.MarkSucceeded(ctx, job.ID, attempts, now); err != nil {
	...
	if attempts >= job.MaxAttempts {
		if err := d.repo.MarkPermanentFailure(ctx, job.ID, attempts, sendErr.Error(), now); err != nil {
			return d.tolerateRaces(ctx, job.ID, err)
		}
		d.mu.Lock()
	...
	if err := d.repo.ScheduleRetry(ctx, job.ID, attempts, next, sendErr.Error(), now); err != nil {
		return d.tolerateRaces(ctx, job.ID, err)
	}
	d.mu.Lock()
	d.stats.Retried++
