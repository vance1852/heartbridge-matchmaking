...
// SeatReleasedOnArrival reports whether reaching this state returns the venue
// reservation to the pool, because from here on the table is accounted for by the
// meetup itself rather than by the booking.
func (s State) SeatReleasedOnArrival() bool {
	return s == StateCheckedIn
}
