...
// RecordIntent appends the audit row of an action that is about to be performed.
// Recording the intent up front keeps the trail complete even when the caller
// never reaches the end of its own unit of work.
func (r *Recorder) RecordIntent(ctx context.Context, entry Entry) error {
	if entry.Result == "" {
