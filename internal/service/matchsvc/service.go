...
// allowanceFailure normalizes every allowance rejection onto one sentinel so
// that callers can recognise it with errors.Is regardless of which rule fired.
func allowanceFailure(memberID string, cause error) error {
	if cause == nil {
		return nil
	}
	return fmt.Errorf(
		"member %s cannot spend an introduction: %w",
		memberID,
		apperr.ErrAllowanceUnavailable,
	)
}
...
func (s *Service) checkAllowance(
	...
	if err := granted.CanReserve(now); err != nil {
		return entitlement.Entitlement{}, 0, allowanceFailure(profile.ID, err)
	}
