...
func (s *Service) Propose(ctx context.Context, actor identity.Actor, input ProposeInput) (Detail, error) {
	...
	return s.propose(ctx, actor, firstID, secondID, nil)
}

// liveCountSnapshot caches how many live introductions each member already takes
...
func (s *Service) propose(
	ctx context.Context, actor identity.Actor, firstID, secondID string, snapshot liveCountSnapshot,
) (Detail, error) {
	prepared, err := s.prepare(ctx, actor, firstID, secondID, snapshot)
	if err != nil {
		return Detail{}, err
	}
...
// activeCount resolves how many live introductions a member already takes part
// in, preferring the caller's snapshot when it carries the member.
func (s *Service) activeCount(
	ctx context.Context, memberID string, snapshot liveCountSnapshot,
) (int, error) {
	if snapshot != nil {
		if count, cached := snapshot[memberID]; cached {
			return count, nil
		}
	}
	return s.matches.CountActiveByMember(ctx, memberID)
}
...
func (s *Service) prepare(
	...
	firstEntitlement, firstWindow, err := s.checkAllowance(ctx, firstProfile, now, snapshot)
	if err != nil {
		return proposal{}, err
	}
	secondEntitlement, secondWindow, err := s.checkAllowance(ctx, secondProfile, now, snapshot)
	if err != nil {
		return proposal{}, err
	}
	window := firstWindow
...
func (s *Service) checkAllowance(
	ctx context.Context, profile member.Member, now time.Time, snapshot liveCountSnapshot,
	...
	active, err := s.activeCount(ctx, profile.ID, snapshot)
	if err != nil {
		return entitlement.Entitlement{}, 0, err
	}
