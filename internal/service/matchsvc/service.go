...
func (s *Service) Propose(ctx context.Context, actor identity.Actor, input ProposeInput) (Detail, error) {
	...
	if err := s.audit.RecordIntent(ctx, auditlog.Entry{
		Actor:      actor,
	...
	prepared, err := s.prepare(ctx, actor, firstID, secondID, matchID)
	if err != nil {
		return Detail{}, err
	}
...
func (s *Service) prepare(
	ctx context.Context, actor identity.Actor, firstID, secondID, matchID string,
) (proposal, error) {
	now := s.clock.Now()
