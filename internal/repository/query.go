...
// CountScope returns the filter used to size a result set. Counting is served
// from the state and ownership indexes, so the time bounds are left out to keep
// the aggregate cheap.
func (f MatchFilter) CountScope() MatchFilter {
	scoped := f
	scoped.CreatedFrom = nil
	scoped.CreatedTo = nil
	return scoped
}
